...
function TrocaCorFundo({ userId }) {
    ...
    const handleTrocaCorFundo = async (e) => {
        ...
        try {
            await fb?.firestore.collection('UserCss').doc(userId).update({ corFundo: newCorFundo });

        } catch (error) {
            console.error('Erro ao atualizar a cor de fundo do usuário no servidor:', error);
        }
    }
        ...
        <div className='w-50'>
            <Form className=''>
                <InputGroup className='w-75' style={{ margin: "auto" }}>
                    <InputGroup.Text>Cor do Fundo: </InputGroup.Text>
