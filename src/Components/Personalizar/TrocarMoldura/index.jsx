...
function TrocarMoldura() {
  ...
  useEffect(() => {
    const fetchMolduras = async () => {
      ...
      urls.unshift({
        name: "sem moldura",
        url: "https://upload.wikimedia.org/wikipedia/commons/a/aa/Empty_set.svg",
      });
      setMolduras(urls);
    };
  ...
  useEffect(() => {
    const updateMoldurasBasedOnRank = async () => {
      ...
      if (
        rank >= 100 &&
        rank <= 399 &&
        !newMolduras.includes("3-molduraprata")
      ) {
        newMolduras.push("3-molduraprata");
      } else if (
        rank >= 400 &&
        rank <= 999 &&
        !newMolduras.includes("4-molduraouro")
      ) {
        newMolduras.push("4-molduraouro");
      } else if (
        rank >= 1000 &&
        rank <= 1999 &&
        !newMolduras.includes("5-molduraplatina")
      ) {
        newMolduras.push("5-molduraplatina");
      } else if (
        rank >= 2000 &&
        rank <= 3999 &&
        !newMolduras.includes("6-molduradiamante")
      ) {
      ...
      if (newMolduras.length !== userMolduras.length) {
        await fb.firestore
          ...
          .doc(authUser.uid)
          .update({ userMolduras: newMolduras });
      }
    };
  ...
  const handleSelectMoldura = async (moldura) => {
    try {
      await fb.firestore
        .collection("UserStats")
        .doc(authUser.uid)
        .update({ moldura: moldura === "sem moldura" ? "" : moldura });
    } catch (error) {
    ...
    <Card bg="secondary" text="light" className="m-0 p-0 h-100">
      ...
      >
        {filledMolduras.map((moldura, index) => (
          ...
          >
            {moldura.name ? (
              moldura.name === "sem moldura" ||
              userMolduras.includes(moldura.name) ? (
                <img
                  title={moldura.name}
                  src={moldura.url}
                  alt={moldura.name}
