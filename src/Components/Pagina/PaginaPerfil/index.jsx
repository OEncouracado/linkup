...
function PaginaPerfil({ username, perfil, selectedFrame, userStyle }) {
    ...
    console.log("estilo username", selectedFrame);
    return (
        <div className='perfilsuperiorwarp d-flex bg-alert flex-column justify-content-center'>
            <div className='frameepefilwarp d-flex justify-content-center align-items-center'>
                {selectedFrame && (<img src={selectedFrame} alt={selectedFrame} className='paginaMolduraPerfil' />)}
                <img src={perfil ? perfil : perfilNull} alt="imagem de perfil" srcSet="" className='paginaImagePerfil rounded-circle' />
            </div>
            <div
